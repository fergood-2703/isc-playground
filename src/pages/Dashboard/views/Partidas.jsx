            ...
            <div className="phase-picker">
              ...
              <div>
                {tournamentPhases.map((phase) => (
                  <button type="button" className={matchForm.phaseType === phase ? "active" : ""} key={phase} onClick={() => setMatchForm({ ...matchForm, phaseType: phase })}>{phase}</button>
                ))}
              </div>
            </div>
            <label>Ronda / nombre
          ...
          {gameMatches.map((match) => {
            ...
            return (
              <article className="match-admin-card" key={match.id}>
                <header>
                  <div><strong>{match.stage}</strong><span>{match.phaseType} · {match.map} · {match.duration} min · {match.scheduledAt}</span><small>Ranking resultante: {leader ? `@${playersById[leader.playerId]?.username} lidera la partida` : "pendiente de puntos"}</small></div>
