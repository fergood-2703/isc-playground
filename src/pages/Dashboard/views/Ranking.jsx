    ...
    <div className="admin-page ranking-admin">
      <div className="page-head">
        <div>
          ...
          <p>El ranking global y por juego usa puntos individuales, estadísticas del usuario e historial de partidas. Los equipos solo organizan rondas.</p>
        </div>
      </div>
      ...
      <div className="podium-grid">
        {globalLeaderboard.slice(0, 3).map((row, index) => (
          <article className={`podium-card rank-${index + 1}`} key={row.player.id}>
            ...
            <strong>{Math.round(row.score)} pts</strong>
          ...
          <tbody>
            {ranking.map((row) => (
              <tr key={row.player.id}>
                ...
                <td><strong>{row.player.name}</strong><small>@{row.player.username}</small></td>
